...
window.productsInCurrentOrder = [];
window.simulatedSupplierOrders = {};
window.simulatedColombiaOrder = null;
window.currentOrderDetails = null;
window.currentViewingPO = null;
window.currentPurchasingPO = null;

// Constantes para cálculo de cubagem
window.MAX_CUBAGE_20HC = 37.4;
window.MAX_CUBAGE_40HC = 76.0;
...
window.universalClientCodeToProductCodeMap = {};
for (const prodCode in window.productDatabase) {
    const product = window.productDatabase[prodCode];
    if (product.clientCode) {
        window.universalClientCodeToProductCodeMap[product.clientCode.toUpperCase()] = prodCode;
    }
}

// Mapeamento e geração de códigos de fornecedor
window.supplierCodeMap = {};
window.supplierNameMap = {};
window.nextSupplierCode = 22; // Inicia com 22 para os códigos de fornecedor

// Nomes de clientes simulados para atribuição cíclica
window.clientNames = ["Cliente Mexico", "Cliente Chile", "Cliente Paraguai", "Cliente Colombia"];
window.clientIndex = 0;

// Função para atribuir nomes de clientes ciclicamente, evitando "Cliente Colombia" para origem "Colômbia"
window.assignClientName = function(originCountry) {
    let assignedClient = window.clientNames[window.clientIndex % window.clientNames.length];
    ...
    if (originCountry === 'Colômbia' && assignedClient === 'Cliente Colombia') {
        window.clientIndex++; // Pula "Cliente Colombia" para esta origem
        assignedClient = window.clientNames[window.clientIndex % window.clientNames.length];
    }
    window.clientIndex++;
...
window.orders = [
    {
        orderId: 'PED-001', parentOrderId: 'SOL-001', clientFlag: window.assignClientName('Hong Kong'), originCountry: 'Hong Kong',
    ...
    {
        orderId: 'PED-002', parentOrderId: 'SOL-009', clientFlag: window.assignClientName('Colômbia'), originCountry: 'Colômbia',
    ...
    {
        orderId: 'PED-003', parentOrderId: 'SOL-002', clientFlag: window.assignClientName('Hong Kong'), originCountry: 'Hong Kong',
    ...
    {
        orderId: 'PED-004', parentOrderId: 'SOL-010', clientFlag: window.assignClientName('Colômbia'), originCountry: 'Colômbia',
    ...
    {
        orderId: 'PED-005', parentOrderId: 'SOL-003', clientFlag: window.assignClientName('Hong Kong'), originCountry: 'Hong Kong',
    ...
    {
        orderId: 'PED-006', parentOrderId: 'SOL-003', clientFlag: window.assignClientName('Hong Kong'), originCountry: 'Hong Kong',
    ...
    {
        orderId: 'PED-007', parentOrderId: 'SOL-004', clientFlag: window.assignClientName('Colômbia'), originCountry: 'Colômbia',
    ...
    {
        orderId: 'PED-008', parentOrderId: 'SOL-011', clientFlag: window.assignClientName('Hong Kong'), originCountry: 'Hong Kong',
    ...
    {
        orderId: 'PED-009', parentOrderId: 'SOL-005', clientFlag: window.assignClientName('Hong Kong'), originCountry: 'Hong Kong',
    ...
    {
        orderId: 'PED-010', parentOrderId: 'SOL-005', clientFlag: window.assignClientName('Hong Kong'), originCountry: 'Hong Kong',
    ...
    {
        orderId: 'PED-011', parentOrderId: 'SOL-006', clientFlag: window.assignClientName('Hong Kong'), originCountry: 'Hong Kong',
    ...
    {
        orderId: 'PED-012', parentOrderId: 'SOL-012', clientFlag: window.assignClientName('Colômbia'), originCountry: 'Colômbia', // This client will not be "Cliente Colombia"
    ...
    {
        orderId: 'PED-013', parentOrderId: 'SOL-007', clientFlag: window.assignClientName('Colômbia'), originCountry: 'Colômbia',
    ...
    {
        orderId: 'PED-014', parentOrderId: 'SOL-013', clientFlag: window.assignClientName('Hong Kong'), originCountry: 'Hong Kong',
    ...
    {
        orderId: 'PED-015', parentOrderId: 'SOL-008', clientFlag: window.assignClientName('Hong Kong'), originCountry: 'Hong Kong',
    ...
    {
        orderId: 'PED-016', parentOrderId: 'SOL-008', clientFlag: window.assignClientName('Hong Kong'), originCountry: 'Hong Kong',
    ...
    {
        orderId: 'PED-017', parentOrderId: 'SOL-014', clientFlag: window.assignClientName('Hong Kong'), originCountry: 'Hong Kong',
    ...
    {
        orderId: 'PED-018', parentOrderId: 'SOL-015', clientFlag: window.assignClientName('Hong Kong'), originCountry: 'Hong Kong',
    ...
    {
        orderId: 'PED-019', parentOrderId: 'SOL-016', clientFlag: window.assignClientName('Hong Kong'), originCountry: 'Hong Kong',
    ...
    {
        orderId: 'PED-020', parentOrderId: 'SOL-017', clientFlag: window.assignClientName('Hong Kong'), originCountry: 'Hong Kong',
    ...
    {
        orderId: 'SAP-001', parentOrderId: 'SOL-018', clientFlag: window.assignClientName('Colômbia'), originCountry: 'Colômbia', // This client will not be "Cliente Colombia"
...
window.initialGeneratedPurchaseOrders = [
    {
...
window.generatedPurchaseOrders = JSON.parse(JSON.stringify(window.initialGeneratedPurchaseOrders));

// Contadores para IDs de pedidos
window.nextParentRequestCounter = 1;
window.nextPurchaseOrderIdCounter = 0;
window.nextSapOrderIdCounter = 1;

// Inicializa os contadores com base nos dados existentes
window.initializeCounters = function() {
    const maxSolId = window.orders.reduce((max, order) => {
    ...
    window.nextParentRequestCounter = maxSolId + 1;

    const maxPedId = window.orders.reduce((max, order) => {
    ...
    window.nextPurchaseOrderIdCounter = maxPedId + 1;

    const maxSapId = window.orders.reduce((max, order) => {
    ...
    window.nextSapOrderIdCounter = maxSapId + 1;

    window.orders.forEach(order => {
        if (order.supplier) {
            window.getOrGenerateSupplierCode(order.supplier);
        }
    });
}

// Função para obter ou gerar código de fornecedor
window.getOrGenerateSupplierCode = function(supplierName) {
    if (!window.supplierCodeMap[supplierName]) {
        window.supplierCodeMap[supplierName] = String(window.nextSupplierCode);
        window.supplierNameMap[String(window.nextSupplierCode)] = supplierName;
        window.nextSupplierCode += 11; // Incrementa para o próximo código
    }
    return window.supplierCodeMap[supplierName];
}

// Função para obter nome do fornecedor pelo código
window.getSupplierNameFromCode = function(supplierCode) {
    return window.supplierNameMap[supplierCode] || supplierCode;
}

// Função para exibir mensagens modais
window.showMessage = function(title, content, type = 'info', onConfirm = null, onCancel = null) {
...
window.toggleCollapsible = function(headerElement, contentId) {
...
window.displayFileName = function(inputId, displayId) {
...
window.synchronizeOrdersAndPOs = function() {
    window.orders.forEach(order => {
        const correspondingPO = window.generatedPurchaseOrders.find(po => po.id === order.orderId);
        if (correspondingPO) {
